...
# 遍历所有 .xls 文件（递归）
Get-ChildItem -Path $sourceRoot -Recurse -Filter *.xls | Where-Object { $_.Name -notlike '~$*' } | ForEach-Object {
    ...
    # 目标文件名
    $targetFile = Join-Path $targetDir ($_.BaseName + ".xlsx")
    if (Test-Path $targetFile) {
        Write-Host "Already exists: $targetFile"
    } else {
        Write-Host "Converting: $sourceFile -> $targetFile"
        # 用 soffice 转换
        & soffice --headless --convert-to xlsx --outdir $targetDir $sourceFile
        # 检查转换结果
        if (Test-Path $targetFile) {
